...
# This plugin processes a logfile ( -l ) and
# report on the time taken entries for last ( -m ) minutes
use strict;
use warnings;
...
sub parse {
	...
	($Ref->{date},$Ref->{time},$Ref->{host},$Ref->{rtype},$Ref->{file},$Ref->{bytes},$Ref->{port},$Ref->{algo},$Ref->{client},$Ref->{algo},$Ref->{proto},$Ref->{code},$Ref->{algo},$Ref->{algo},$Ref->{timeTaken}) = $Line =~ m,^([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+),;
...
#my $log_file = join "", $folder_path, "/", $prefix_log, $sufix_log;
...
if ( $count > 0) 
{
    # IIS logs in miliseconds (Apache HTTP server logs in microseconds)
	$max = ceil($max);
	$min = ceil($min);
